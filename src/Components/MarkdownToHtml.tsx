...
function MarkdownToHtml({ markdown }: MarkdownToHtmlProps) {
  const markdownToHtml = (markdown: string) => {
    markdown = markdown.replace(
      /^(#{1})\s+(.*)$/gm,
      (_match, _hashes, heading) => {
        const className =
          "text-[32px] font-[700] leading-[42.2px] font-sans mb-[21px]";
    ...
    markdown = markdown.replace(
      ...
      (_match, _hashes, heading) => {
        const className =
          "text-[28px] font-[400] leading-[36.93px] font-sans mb-[20px] mt-[21px]";
    ...
    markdown = markdown.replace(
      ...
      (_match, _hashes, heading) => {
        const className =
          "text-[24px] font-[700] leading-[31.65px] font-sans mb-[20px] mt-[20px]";
    ...
    markdown = markdown.replace(
      ...
      (_match, _hashes, heading) => {
        const className =
          "text-[20px] font-[700] leading-[26.38px] font-sans mb-[20px] mt-[20px]";
    ...
    markdown = markdown.replace(
      ...
      (_match, _hashes, heading) => {
        const className =
          "text-[16px] font-[700] leading-[21.1x] font-sans mb-[20px] mt-[20px]";
    ...
    markdown = markdown.replace(
      ...
      (_match, _hashes, heading) => {
        const className =
          "text-[14px] font-[700] leading-[18.46px] font-sans text-[#E46643] mb-[21px] mt-[20px]";
    ...
    markdown = markdown.replace(/```([\s\S]*?)```/g, (_match, codeBlock) => {
      const className =
        "text-[14px] font-[400] leading-[24px] font-mono w-fit p-[24px] mt-[20px] mb-[20px] text-[#35393F] bg-[#F5F5F5]";
    ...
    markdown = markdown.replace(/^\s*>\s+(.*)$/gm, (_match, quote) => {
      const className =
        "text-[14px] font-[700] leading-[24px] w-fit font-sans bg-[#F5F5F5] p-[24px] mt-[20px] border-l-[4px] border-[#E46643] rounded-md";
    ...
    markdown = markdown.replace(/^\s*-\s+(.*)$/gm, (_match, item) => {
      const className =
        "text-[14px] font-[400] leading-[24px] font-sans text-[#7C8187] ml-[24px] list-disc marker:text-[#E46643]";
      return `<li class="${className}">${item}</li>`;
    });

    markdown = markdown.replace(/^\d+\.\s+(.*)$/gm, (_match, listItem) => {
      const className =
        "list-decimal text-[14px] font-[400] leading-[24px] font-sans text-[#7C8187] ml-[24px]";
    ...
    markdown = markdown.replace(
      ...
      (_match, before, text) => {
        ...
        const className =
          "text-[14px] font-[400] leading-[24px] font-sans w-fit pr-[24px] text-[#7C8187]";
